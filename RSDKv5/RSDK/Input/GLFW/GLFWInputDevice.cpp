...
void RSDK::SKU::InitGLFWInputAPI()
{
    ...
    f.externalFile = true;
    char path[0x100]; 
    sprintf_s(path, sizeof(path), "%sgamecontrollerdb.txt", SKU_userFileDir);
    ...
    if (LoadFile(&f, path, FMODE_RB)) {
        ...
        PrintLog(PRINT_NORMAL, "[GLFW] gamecontrollerdb size %d: %s", f.fileSize, buf);

        if (glfwUpdateGamepadMappings(buf) == GLFW_FALSE)
            PrintLog(PRINT_NORMAL, "[GLFW] failed to load from gamecontrollerdb");
    ...
    for (int32 i = GLFW_JOYSTICK_1; i < GLFW_JOYSTICK_LAST; ++i) {
        PrintLog(PRINT_NORMAL, "[GLFW] joystick %d is '%s', isGamepad %d", i, glfwGetJoystickName(i), glfwJoystickIsGamepad(i));
