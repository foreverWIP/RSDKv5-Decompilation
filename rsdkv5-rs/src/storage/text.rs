...
pub fn gen_hash_md5(message: &str) -> HashMD5 {
    unsafe {
        ...
        for i in 0..4 {
            ...
            *buffer_bytes.wrapping_add(i * 4 + 1) = md5.0[i * 4 + 1];
...
#[no_mangle]
#[export_name = "GenerateHashCRC"]
pub extern "C" fn gen_hash_crc(id: *mut uint32, mut input_string: *mut u8) {
    unsafe {
        *id = u32::MAX;
        ...
        loop {
            len += 1;
            if *input_string.wrapping_add(len) == 0 {
                break;
            }
        }

        for _ in 0..len {
            *id = CRC32_T[((*id ^ *input_string as u32) as uint8) as usize] ^ (*id >> 8);
